            ...
            <%
                if(request.getParameter("type")!=null) {
                    try(Connection conn = DriverManager.getConnection(url,user,pass)) {
                        ...
                        if(type.equals("sale")) {
                            out.println("<br><br><h4>Results for 'Sale' between " + request.getParameter("Minimum") + "/- and " + request.getParameter("Maximum") + "/-</h4>");
                            query = "select house_id,agent_name,agents.contact_number,city,street_name,apt_no,sale_price,no_bedrooms,specificatons from house natural join agents where sale_status = 'N' and (rent_status is null or rent_status = 'N')  and sale_price between ? and ?";
                            stmt = conn.prepareStatement(query);
                            stmt.setString(1, request.getParameter("Minimum"));
                            stmt.setString(2, request.getParameter("Maximum"));
                        } else {
                            out.println("<br><br><h4>Results for 'Rent' between " + request.getParameter("Minimum") + "/- and " + request.getParameter("Maximum") + "/-</h4>");
                ...
                <table class="table-bordered table table-striped table-hover">
                    ...
                    <tbody>
                        <%
                            while(rset.next()) {
                                out.println("<tr>");
                                out.print("<td><img src='https://cdn.houseplansservices.com/content/oanrn2hpo2onko9gr94416qock/w991x660.jpg?v=10' alt='House Image'  style='width: 150px; height: 100px;'></td>");
                                for(int i=1; i<=columnCount; i++) {
